...
export const generatePnLDataForMonth = (monthName: string, year: number, targetTotalPnL: number): DailyPnL[] => {
  ...
  // Dữ liệu đặc biệt cho tháng 02/2026 theo yêu cầu người dùng
  if (isFeb2026) {
    const data: DailyPnL[] = [];
    const feb2026Results = [
      { percentage: 2.57, isProfit: false }, // Ngày 1
      { percentage: 2.60, isProfit: true },  // Ngày 2
    ...
    for (let i = 1; i <= daysInMonth; i++) {
      ...
      if (i <= feb2026Results.length) {
        const res = feb2026Results[i - 1];
        data.push({
          date: dateStr,
          percentage: res.percentage,
          isProfit: res.isProfit,
          strategy: 'Alpha-X AI V4.0'
        });
      } else {
        data.push({
          date: dateStr,
  ...
  // Logic tạo dữ liệu giả lập cho các tháng khác
  ...
  const currentDayLimit = isFutureMonth ? 0 : daysInMonth;
